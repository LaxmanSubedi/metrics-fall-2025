...
# Implementing the four nested loops for backward recursion.
@views @inbounds function compute_future_value!(FV, θ, d) # views and inbounds for performance
    ...
    for t in d.T:-1:1
        #Loop over brand states
         for b in 0:1 # Brand status in flow utility        
             ...
             for z in 1:d.zbin
                #Loop over mileage states
                 for x in 1:d.xbin
                     ...
                     # Where FV(t) = β·log(exp(v₀) + exp(v₁))
                     # This is the expected value before the agent observes ε
                     FV[row, b+1, t] = d.β * log(exp(v1) + exp(v0)) + d.β * Base.MathConstants.eulergamma  # Adding Euler-Mascheroni constant for logit
               end
            end
        end
    end
   ...
   for i in 1:d.N
        ...
        for t in 1:d.T
            ...
            row1 = d.Xstate[i,t] + (d.Zstate[i] - 1) * d.xbin
             ...
             flow_diff = θ[1] + θ[2]*d.X[i,t] + θ[3]*d.B[i] # already difference
            ...
            v_diff = flow_diff + d.β * ev_diff # no need eulergamma as v's are differenced
            
            # Add to log likelihood
            # Efficient form: ℓ = Σ[Y·v_diff - log(1 + exp(v_diff))]
             loglike += (d.Y[i,t] == 1) * v_diff - log(1 + exp(v_diff)) # everything scalar
