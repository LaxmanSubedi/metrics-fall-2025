...
function mlogit_with_Z(theta, X, Z, y)
    ...
    # we need to Compute choice probabilities
...
function variance_quadrature()
   ...
   println("more grid points give better approximation ")
...
function practice_monte_carlo()
    ...
    function mc_integrate(f, a, b, D)
        ...
        draws = rand(D) * (b - a) .+ a  # uniform draws on [a,b] no dot because of scaler
...
function mixed_logit_quad(theta, X, Z, y, R)# R
    # Extract parameters
    ...
    nodes, weights = lgwt(R, mu_gamma-5*sigma_gamma, mu_gamma+5*sigma_gamma) #R

    # Initialize integrated probabilities
    T = promote_type(eltype(X), eltype(theta))
    P_integrated = zeros(T, N, J)
    
    # loop through frid points to do summation via loop
...
function optimize_mixed_logit_mc(X, Z, y)
    ...
    println("Starting values: μ_γ = $(startvals[end-1]), σ_γ = $(startvals[end])")
    return startvals  # Return starting values instead of running
end
