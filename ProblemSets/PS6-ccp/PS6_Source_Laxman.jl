#Read the file to compute the transition probability
...
function load_and_reshape_data(url::String)
    ...
    #------------------------------------------------------------
    # Reshape from wide to long (done separately for each variable
    # because stack() requires doing one variable set at a time)
    #------------------------------------------------------------
...
function main()
    ...
    # Define grid parameters like that of in PS5 (xbin, zbin, xval, zval)
